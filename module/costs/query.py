...
def get_costs_detail(db_path,subject_name):
    #若传入参数带引号，则去除引号
    my_path = rf'{db_path}' 
    ...
    WITH t1 AS 
    (
    SELECT 总账科目名称,公司代码货币价值,总账科目编码,
    CASE WHEN CAST(记帐期间 AS int)>12 THEN '13期调整' ELSE  concat(CAST(CAST(记帐期间 AS int) AS VARCHAR),'月') END AS 期间_adj
    FROM 费用发生额
    WHERE 功能范围='{subject_name}'
    UNION ALL 
    SELECT 'temp' AS 总账科目名称,0 AS 公司代码货币价值,'temp' 总账科目编码,CONCAT(CAST(UNNEST(generate_series(1, 12))AS VARCHAR),'月') 期间_adj
    UNION ALL 
    SELECT 'temp' AS 总账科目名称,0 AS 公司代码货币价值,'temp' 总账科目编码,'13期调整' 期间_adj
    ),
    t2 AS 
    (
    PIVOT t1
    ON 期间_adj
    USING sum(公司代码货币价值)
    GROUP BY 总账科目编码,总账科目名称
    )
    SELECT 
    row_number() over(ORDER BY 总账科目编码 ASC) 序号,
    总账科目编码 AS 编码,
    总账科目名称 AS 项目,
    ...
    #添加合计列
    df['合计']=df.iloc[:,3:].apply(sum,axis=1).round(2)

    #插入两个空行
    temp_list = [0 for i in range(len(df.columns))]
    df.loc[len(df)]=temp_list
    df.loc[len(df)]=temp_list

    #添加合计行
    temp_sum=df.iloc[:,3:].map(lambda x:round(x,2)).apply(sum,axis=0).round(2)
    df.iloc[-1,3:] = temp_sum
    df.iloc[-1,2] = '合计'
    df.iloc[-1,:2] = None

    # #把空行换成None
    df.iloc[-2,:] = [None for i in range(len(df.columns))]

    return df


def get_fin_costs(db_path):
     #若传入参数带引号，则去除引号
    my_path = rf'{db_path}' 
    if my_path[0]=='"' or my_path[0]=="'":
        my_path = my_path[1:-1]
...
if __name__ == '__main__':
    ...
    db_path = r'D:\audit_project\DEV\auto_workingpaper\test\test_costs'
    subject_name = '销售费用'
    df = get_costs_detail(db_path,subject_name)
    # print(df)
    xw.view(df)
    # df = get_fin_costs(db_path,subject_name)
